...
class CharRNN:
    ...
    def build_lstm(self):
        ...
        with tf.name_scope('lstm'):
            ...
            self.bw_state = cell.zero_state(self.num_seqs,tf.float32)

            # self.lstm_outputs, self.final_state = tf.nn.dynamic_rnn(cell, self.lstm_inputs, initial_state=self.initial_state)
            outputs,states = bi_rnn(cell,cell,inputs=self.lstm_inputs,initial_state_fw = self.initial_state,
                                    initial_state_bw=self.bw_state,dtype=tf.float32)

            self.lstm_outputs = outputs[0]
            self.final_state = states[0]
            seq_output = tf.concat(self.lstm_outputs, 1)
            x = tf.reshape(seq_output, [-1, self.lstm_size])

            # self.bw_outputs = outputs[1]
            self.last_state = states[1]
            # bw_output = tf.concat(self.bw_outputs, 1)
            # x1 = tf.reshape(bw_output, [-1, self.lstm_size])

            with tf.variable_scope('softmax'):
                softmax_w = tf.Variable(tf.truncated_normal([self.lstm_size, self.num_classes], stddev=0.1))
                # softmax_w1 = tf.Variable(tf.truncated_normal([self.lstm_size, self.num_classes], stddev=0.1))
                softmax_b = tf.Variable(tf.zeros(self.num_classes) )

            self.logits = tf.matmul(x, softmax_w) + softmax_b
            # self.logits = tf.add(tf.add(tf.matmul(x, softmax_w), tf.matmul(x1, softmax_w1)),softmax_b)
    ...
    def train(self, batch_generator, save_path):
        ...
        with self.session as sess:
            ...
            bw_state = sess.run(self.bw_state)
            for x, y in batch_generator:
                ...
                accuracy,batch_loss, new_state, _ ,bw_state= sess.run([self.accuracy,self.loss,
                                                     self.final_state,
                                                     self.optimizer,
                                                      self.last_state  ],
                                                    feed_dict=feed)
    ...
    def sample(self,prime, vocab_size):
        ...
        for c in prime:
            ...
            feed = {self.inputs: x,
                    self.keep_prob: 1.,
                    self.initial_state: new_state,self.bw_state :bw_state}
            preds, new_state,bw_state = sess.run([self.proba_prediction, self.final_state,self.last_state],
                                        feed_dict=feed)
        top_n = pick_top_n(preds, vocab_size)
        return top_n
